...
import { $, addDisposableListener, EventType, EventHelper, addStandardDisposableListener, clearNode, append, isAncestor } from 'monaco-editor/esm/vs/base/browser/dom.js';
...
import { asCssVariable, buttonBackground, buttonSecondaryBackground, editorErrorForeground } from 'monaco-editor/esm/vs/platform/theme/common/colorRegistry.js';
import { toWorkspaceIdentifier, IWorkspaceContextService } from 'monaco-editor/esm/vs/platform/workspace/common/workspace.js';
...
import { LIST_WORKSPACE_UNSUPPORTED_EXTENSIONS_COMMAND_ID, IExtensionsWorkbenchService } from '../../extensions/common/extensions.js';
...
import { posix, win32 } from 'monaco-editor/esm/vs/base/common/path.js';
import { hasDriveLetter, toSlashes } from 'monaco-editor/esm/vs/base/common/extpath.js';
...
const folderPickerIcon = registerIcon('workspace-trust-editor-folder-picker', Codicon.folder, ( (localize(
    'folderPickerIcon',
...
const removeIcon = registerIcon('workspace-trust-editor-remove-folder', Codicon.close, ( (localize(
    'removeIcon',
...
let WorkspaceTrustEditor = class WorkspaceTrustEditor extends EditorPane {
    ...
    getHeaderTitleText(trusted) {
        if (trusted) {
            if (this.workspaceTrustManagementService.isWorkspaceTrustForced()) {
                return (
                     (localize('trustedUnsettableWindow', "This window is trusted"))
                );
            }
            switch (this.workspaceService.getWorkbenchState()) {
                case 1 :
                    return (
                         (localize('trustedHeaderWindow', "You trust this window"))
                    );
                case 2 :
                    return (
                         (localize('trustedHeaderFolder', "You trust this folder"))
                    );
                case 3 :
                    return (
                         (localize('trustedHeaderWorkspace', "You trust this workspace"))
                    );
            }
        }
        return (
             (localize('untrustedHeader', "You are in Restricted Mode"))
        );
    }
    ...
    getExtensionCount() {
        ...
        for (const extension of localExtensions) {
            ...
            if (enablementState !== 8  && enablementState !== 9  &&
                enablementState !== 0  && enablementState !== 5 ) {
                continue;
            }
    ...
    async renderAffectedFeatures(numSettings, numExtensions) {
        ...
        const untrustedContainerItems = this.workspaceService.getWorkbenchState() === 1  ?
            ...
            [
                ( (localize('untrustedTasks', "Tasks are not allowed to run"))),
                ( (localize('untrustedDebugging', "Debugging is disabled"))),
                fixBadLocalizedLinks(numSettings ? ( (localize(
    ...
    addTrustedTextToElement(parent) {
        ...
        if (!this.workspaceTrustManagementService.isWorkspaceTrustForced()) {
            textElement.innerText = this.workspaceService.getWorkbenchState() === 3  ? ( (localize(
...
WorkspaceTrustEditor.__decorator = ( (__decorate([
    debounce(100)
...
WorkspaceTrustEditor = WorkspaceTrustEditor_1 = ( (__decorate([
    ...
    ( (__param(8, IWorkbenchConfigurationService))),
...
export { WorkspaceTrustEditor, shieldIcon };
