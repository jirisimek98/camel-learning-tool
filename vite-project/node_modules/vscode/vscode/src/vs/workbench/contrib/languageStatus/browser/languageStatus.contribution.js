...
let EditorStatusContribution = class EditorStatusContribution {
    ...
    constructor(_languageStatusService, _statusBarService, _editorService, _openerService, _storageService) {
        ...
        _storageService.onDidChangeValue(0 , EditorStatusContribution_1._keyDedicatedItems, this._disposables)(this._handleStorageChange, this, this._disposables);
        this._restoreState();
    ...
    _restoreState() {
        const raw = this._storageService.get(EditorStatusContribution_1._keyDedicatedItems, 0 , '[]');
        try {
    ...
    _storeState() {
        if (this._dedicated.size === 0) {
            this._storageService.remove(EditorStatusContribution_1._keyDedicatedItems, 0 );
        }
        else {
            const raw = JSON.stringify(Array.from(( this._dedicated.keys())));
            this._storageService.store(EditorStatusContribution_1._keyDedicatedItems, raw, 0 , 0 );
        }
    }
    ...
    _update() {
        ...
        if (model.combined.length === 0) {
            ...
            if (!this._combinedEntry) {
                this._combinedEntry = this._statusBarService.addEntry(props, EditorStatusContribution_1._id, 1 , { id: 'status.editor.mode', alignment: 0 , compact: true });
            }
            else {
